    ...
    if (input) {
      ...
      const moveMemberToNextChannel = (index) => {
        if (index >= sortedVoiceChannels.length) {
          // Finished moving through all channels, move to "Boden" channel
          const bodenChannel = interaction.guild.channels.cache.find(
            channel => channel.name.toLowerCase() === 'boden'
          );

          if (bodenChannel) {
            setTimeout(() => {
              member.voice.setChannel(bodenChannel);
              interaction.followUp({
                content: `Moved ${member.displayName} to ${bodenChannel.name}.`,
                ephemeral: true,
              });
            }, 1000); // 1000 milliseconds = 1 second
          } else {
            interaction.followUp('Target "Boden" voice channel not found.');
          }

          return;
        }

        //The Move Sequenze throught the "Fall" Channels
